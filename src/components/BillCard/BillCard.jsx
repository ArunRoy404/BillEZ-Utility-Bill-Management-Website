    ...
    return (
        <div className="flex flex-col sm:flex-row gap-10 md:gap-15 lg:gap-20 bg-gray-900 text-white border border-gray-200 rounded-xl shadow-sm py-8 px-20 hover:shadow-md transition-shadow mx-auto">
            ...
            <div className='flex flex-col justify-between'>
                <div>
                    <div>
                        <h3 className="font-bold text-2xl md:text-4xl md:w-80">{bill.organization}</h3>
                    </div>

                    <div className="mt-1 mb-2 ">
                        <p className="text-sm font-bold opacity-70 capitalize">{bill.bill_type}</p>
                ...
                <div className='flex items-center gap-15'>
                    <div>
                        <span className="md:text-xl font-bold text-blue-400">BDT {bill.amount}</span>
                    </div>

                    <Link to={`/bills/bill-${bill.id}`} className="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group">
