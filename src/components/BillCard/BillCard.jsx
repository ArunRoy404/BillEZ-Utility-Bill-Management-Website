...
const BillCard = ({ bill }) => {
    ...
    const payBtn = (
        <Link to={`/bills/${bill.id}`} className="relative inline-flex items-center justify-start py-2 md:py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group">
        ...
        <div className="flex flex-row gap-8 md:gap-15 lg:gap-20 bg-gray-900 text-white border border-gray-200 rounded-xl shadow-sm p-3 md:py-8 md:px-20 hover:shadow-md transition-shadow mx-auto">
            <div className="flex items-center md:border rounded-2xl overflow-hidden max-w-max mx-auto">
                <img
                    ...
                    className="rounded-xl w-30 md:w-40 md:h-40 object-cover bg-white sm:p-8"
                />
            </div>
            ...
            <div className='flex flex-1 flex-col justify-between'>
                ...
                <div className={`flex items-center ${bill.status == "due" && 'gap-3 md:gap-15'}`}>
                    <div>
                        {bill.status == "due" && <span className="text-sm md:text-xl font-bold text-blue-400">BDT {bill.amount}</span>}
                    </div>
                    {
                        bill.status == 'due'
                            ? payBtn
